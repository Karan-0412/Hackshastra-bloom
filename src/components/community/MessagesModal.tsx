...
const MessagesModal: React.FC<{ open: boolean; onOpenChange: (v: boolean) => void; partner?: { id: string; name: string; avatarUrl?: string | null } }> = ({ open, onOpenChange, partner }) => {
  ...
  }, [people, search, threads, currentUser]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      people.forEach(p => { if (!next[p.id]) next[p.id] = { online: Math.random() < 0.5, lastSeen: new Date(Date.now() - Math.floor(Math.random()*6_000_000)).toISOString() }; });
      if (currentUser) next[currentUser.id] = { online: true, lastSeen: new Date().toISOString() };
      setOnlineMap(next);
      localStorage.setItem(STATUS_KEY, JSON.stringify(next));
    } catch {}
  }, [open, people, currentUser?.id]);
        ...
        <div className="grid grid-cols-[320px_minmax(0,1fr)] h-[70vh] gap-0">
          ...
          <div className="flex flex-col">
            ...
            <div className="px-4 py-3 border-b flex items-center justify-between">
              <div>
                <div className="font-semibold">{selected ? people.find(p => p.id === selected)?.name || 'Chat' : 'Select a person'}</div>
                {selected && (
              ...
              {selected && (
                <Avatar className="h-8 w-8">
                  <AvatarImage src={people.find(p => p.id === selected)?.avatarUrl || undefined} />
                  <AvatarFallback>{people.find(p => p.id === selected)?.name.charAt(0)}</AvatarFallback>
                </Avatar>
              )}
            </div>
